...
export default function DeckEditor({ deck, onNavigate }) {
    ...
    const [ modalConfig, setModalConfig ] = useState({ isOpen: false, mode: "create", card_id: null, initial_value: { question: "", answers: [], options: [] } });
    const [ isCardError, setIsCardError ] = useState(false);

    const openCreateModal = () => {
        setModalConfig({ isOpen: true, mode: "create", card_id: null, initial_value: { question: "", answers: [], options: [] } });
    }

    const openEditModal = (id, question, answers, options) => {
        setModalConfig({ isOpen: true, mode: "edit", card_id: id, initial_value: { question, answers, options } });
    }

    const handleCardCreation = async (question, answers, options) => {
        const res = await createCard(deck.id, question, answers, options);
        if (!res.ok) {
            return;
        }
    ...
    const handleCardEdit = async (id, question, answers, options) => {
        const res = await editCard(id, question, answers, options);
        if (!res.ok) {
            return;
        }
    ...
    const handleSaveCard = async (id, question, answers, options) => {
        if (modalConfig.mode === "create") {
            await handleCardCreation(question, answers, options);
        } else if (modalConfig.mode === "edit") {
            await handleCardEdit(id, question, answers, options);
        }
    }
        ...
        <>
            <div className="w-full min-h-screen flex flex-col text-white">
                <div className="inline-flex w-full bg-dungeon-dark-900 items-center justify-between">
                    <h1 className="text-4xl font-pixel-header font-bold p-5">Cards of <span className="text-dungeon-orange-glow">{deck.name}</span>: </h1>
                ...
                <div className="flex flex-col grow relative bg-dungeon-dark-900">
                    <div className="grid grid-cols-4 place-items-center font-pixel-header text-4xl *:p-5">
                        <p>Type</p>
                        <p>Question</p>
                        <p>Answers</p>
                    ...
                    {cards.map((card) => (
                        <div key={card.id} className="grid grid-cols-4 text-xl items-center m-4 *:p-2 *:m-2 mb-2 border border-dungeon-yellow divide-x-2 divide-dungeon-yellow">
                                <span>{card.options.length > 1 ? "Multiple Choice" : "Input Field"}</span>
                                <span>{card.question}</span>
                                <div>
                                    {card.answers.map((answer) => (
                                        <p key={answer}>- {card.options.at(answer)}</p>
                                    ))}
                                </div>
                                <div className='*:p-2 *:rounded-xl *:hover:bg-dungeon-yellow *:transition-colors flex justify-center md:flex-row flex-col gap-2'>
                                    ...
                                    <button className="bg-dungeon-purple grow" onClick={() => openEditModal(card.id, card.question, card.answers, card.options)}>Edit</button>
                                </div>
                        </div>
                    ))}
                    {cards.length === 0 && (
